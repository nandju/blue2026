	...
	return (
		<>
			<motion.div
				...
				initial={false}>
				<div className="relative opacity-95 flex flex-col items-center space-x-8 min-h-[100vh] bg-[#0D6EBB] min-w-[100vw] ">
		...
		<>
			<nav
				...
				className={`navbar px-5 md:px-24 w-screen fixed transition-colors ease duration-500 ${
					isNavOpen
						? "backdrop-filter backdrop-blur-md bg-[#0D6EBB] bg-opacity-50"
				...
				<div>
					<h1
						className={`text-2xl ml-2 md:ml-0 transition-colors ease duration-500 ${
							isNavOpen ? "text-white" : "text-[#0D6EBB]"
						}`}>
						BLUE
					</h1>
				</div>
					...
					<button
						...
						<div
							className={`w-10 h-1 rounded-full transition-all ease duration-300 ${
								isNavOpen ? "rotate-45 bg-white translate-y-[2px]" : "bg-[#0D6EBB]"
							}`}></div>
						<div
							className={`w-10 h-1 rounded-full transition-all ease duration-300 ${
								isNavOpen ? "-rotate-45 -translate-y-2 bg-white" : "bg-[#0D6EBB]"
							}`}></div>
					</button>
				</div>
			</nav>
