...
const CHAT_USERS_KEY = 'blue_chat_users';
const CHAT_MESSAGES_KEY = 'blue_chat_messages';

export const getChatUsers = () => get(CHAT_USERS_KEY) || [];

export const addChatUser = (userData) => {
  const users = getChatUsers();
  const existing = users.find((u) => u.sessionId === userData.sessionId);
  ...
  users.push(user);
  set(CHAT_USERS_KEY, users);
  return user.id;
};

export const getChatUser = (id) => getChatUsers().find((u) => u.id === id) || null;
export const getChatUserBySession = (sessionId) => getChatUsers().find((u) => u.sessionId === sessionId) || null;
export const updateChatUser = (sessionId, updates) => {
  const users = getChatUsers();
...
export const getChatMessages = (userId) => {
  const all = get(CHAT_MESSAGES_KEY) || {};
  return all[userId] || [];
};
...
export const addChatMessage = (userId, userMessage, aiResponse) => {
  ...
  all[userId].push({ id: Date.now(), userMessage, aiResponse, createdAt: new Date().toISOString() });
  set(CHAT_MESSAGES_KEY, all);
};

export const getAllChatMessages = () => {
  const all = get(CHAT_MESSAGES_KEY) || {};
  return Object.entries(all).flatMap(([userId, msgs]) => msgs.map((m) => ({ ...m, userId })));
};
...
export const getChatStats = () => {
  ...
  return { totalUsers: users.length, totalMessages: messages.length, newToday, topLocations };
};
...
export const getStats = () => ({
  ...
  conversations: getChatUsers().length,
  volunteers: 1,
});
